...
- One file per feature area (`validations.test.ts`, `callbacks.test.ts`, `associations.test.ts`, `dirty.test.ts`, `casting.test.ts`, `macros.test.ts`, `scopes.test.ts`, `transactions.test.ts`, `convenience.test.ts`, `queries.test.ts`, `attributes.test.ts`, `attributeModel.test.ts`, plus `decorators.test.ts`/`errors.test.ts` for pure-metadata/unit-level checks). Add new tests to the matching file rather than creating a new one per test case.
